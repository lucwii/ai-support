    ...
    <div className="p-5">
      ...
      <div className="grid grid-cols-4 gap-3 mb-5">
        {[
          ...
          <div key={s.label} className="rounded-xl p-3.5" style={{ background: '#0A0F1E', border: '1px solid rgba(255,255,255,0.06)' }}>
            <p className="text-[11px] mb-1" style={{ color: '#475569' }}>{s.label}</p>
            <p className="text-[20px] font-sora font-bold" style={{ color: s.color }}>{s.value}</p>
          </div>
        ))}
      </div>
      ...
      <div className="rounded-xl overflow-hidden" style={{ border: '1px solid rgba(255,255,255,0.06)' }}>
        ...
        {sampleTickets.map((ticket, i) => (
          ...
          >
            ...
            <p className="text-[13px] pr-4 truncate" style={{ color: '#CBD5E1' }}>{ticket.content}</p>
            <ConfBadge value={ticket.confidence} />
            <span className="text-[12px]" style={{ color: '#475569' }}>{ticket.time}</span>
          </div>
        ))}
      </div>
    </div>
...
function AIResponseView() {
  return (
    <div className="p-5 grid md:grid-cols-[60%_40%] gap-4">
      {/* Left */}
    ...
    <div className="p-5">
      ...
      {['Our return policy allows returns within 30 days of purchase...', 'Shipping typically takes 3-5 business days for standard delivery...', 'To reset your password, go to Settings → Security → Reset Password...'].map((chunk, i) => (
        <div key={i} className="flex items-center gap-3 px-3 py-2.5 rounded-lg mb-2" style={{ background: '#0A0F1E', border: '1px solid rgba(255,255,255,0.04)' }}>
          <span className="text-[11px] font-jb" style={{ color: '#475569' }}>#{i + 1}</span>
...
export default function ProductDemo() {
  ...
  return (
    <section className="py-[120px] relative overflow-hidden" style={{ background: 'var(--bg-navy)' }}>
      {/* Glow behind browser */}
      <div
        ...
        <ScrollReveal>
          ...
          <div className="flex gap-2 mb-4 justify-center flex-wrap">
            {TABS.map((tab) => (
              <button
                ...
                className="text-[13px] font-medium px-5 py-2 rounded-lg transition-all duration-150"
                style={{
          ...
          >
            ...
            >
              ...
              <div
                className="flex-1 mx-4 py-1.5 px-3 rounded-md font-jb text-[12px] text-center"
            ...
            {/* Content */}
            <div style={{ background: '#080C18', minHeight: 320 }}>
